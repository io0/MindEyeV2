...
if utils.is_interactive():
    ...
    jupyter_args = f"--data_path=./data \
                    --cache_dir=./data \
                    --model_name=40sesstry1 \
                    --no-multi_subject --subj=1 --batch_size=8 --num_sessions=40 \
...
for iter, (behav0, past_behav0, future_behav0, old_behav0) in enumerate(test_dl):
  break
...
# behav0[:,0,0].cpu().long().float()


# In[28]:


# ind = behav0[:,0,5].cpu().long()
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=data_type): 
            for test_i, (behav, past_behav, future_behav, old_behav) in enumerate(test_dl):  
                ...
                if test_image is None:
                    # if downsampled:
                    #     ind = behav[:,0,5].cpu().long()
                    #     behav = behav[np.where(ind < 27750)[0]]
